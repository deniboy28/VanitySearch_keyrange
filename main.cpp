...
void printUsage() {
  ...
  printf("             [-nosse] [-r rekey] [-startkey hex_private_key] [-endkey hex_private_key] [-check] [-kp] [-sp startPubKey]\n");
  ...
  printf(" -startkey hex_private_key: Start the search from a specific private key (hex format)\n");
  printf(" -endkey hex_private_key: End the search at a specific private key (hex format)\n");
...
int main(int argc, char* argv[]) {
  ...
  string startKeyHex = "";
  bool startKeySpecified = false;
  string endKeyHex = "";
  bool endKeySpecified = false;
  ...
  printf("GPU code not compiled, use -DWITHGPU when compiling.\n");
      ...
      printf("Priv (HEX): 0x%s\n", k.GetBase16().c_str()); // Added this line
      ...
      a++;
    } else if (strcmp(argv[a], "-startkey") == 0) { // New option
      a++;
      startKeyHex = string(argv[a]);
      startKeySpecified = true;
      if (startKeyHex.length() != 64) { // Validation check
        printf("Error: -startkey must be a 64-character hexadecimal private key.\n");
        exit(-1);
      }
      a++;
    } else if (strcmp(argv[a], "-endkey") == 0) { // New option
      a++;
      endKeyHex = string(argv[a]);
      endKeySpecified = true;
      if (endKeyHex.length() != 64) { // Validation check
        printf("Error: -endkey must be a 64-character hexadecimal private key.\n");
        exit(-1);
      }
      a++;
  ...
  VanitySearch *v = new VanitySearch(secp, prefix, seed, searchMode, gpuEnable, stop, outputFile, sse,
    maxFound, rekey, caseSensitive, startPuKey, paranoiacSeed, startKeyHex, startKeySpecified, endKeyHex, endKeySpecified);
