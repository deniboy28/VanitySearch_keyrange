...
VanitySearch::VanitySearch(Secp256K1 *secp, vector<std::string> &inputPrefixes,string seed,int searchMode,
                           ...
                           std::string startKeyHex, bool startKeySpecified,
                           std::string endKeyHex, bool endKeySpecified)
  ...
  this->startKeyFromCmdLineSpecified = startKeySpecified;
  if (startKeySpecified) {
    this->startKeyFromCmdLine.SetBase16((char *)startKeyHex.c_str());
  }
  this->endKeyFromCmdLineSpecified = endKeySpecified;
  if (endKeySpecified) {
    this->endKeyFromCmdLine.SetBase16((char *)endKeyHex.c_str());
  }
  ...
  // Seed
  if (startKeyFromCmdLineSpecified) {
    startKey.Set(&startKeyFromCmdLine); // Use command line start key
  } else {
    if (seed.length() == 0) {
      // Default seed
...
bool VanitySearch::checkPrivKey(string addr, Int &key, int32_t incr, int endomorphism, bool mode) {
  ...
  // Final check: ensure the found key is within the user's specified range.
  if (endKeyFromCmdLineSpecified) {
    if (k.IsLower(&startKeyFromCmdLine) || k.IsGreater(&endKeyFromCmdLine)) {
      return false; // It's a valid key for the prefix, but it's out of the desired range.
    }
  }
...
void VanitySearch::getCPUStartingKey(int thId,Int& key,Point& startP) {
  ...
  if (rekey > 0) {
    ...
    if (startKeyFromCmdLineSpecified) { // Use startKey from command line
      key.Set(&startKeyFromCmdLine);
    } else { // Use seed-derived startKey
      key.Set(&startKey);
    }
    // Only apply thread offset if we are not doing a range search
    if (!endKeyFromCmdLineSpecified) {
...
void VanitySearch::FindKeyCPU(TH_PARAM *ph) {
  ...
  while (!endOfSearch) {
    if (endKeyFromCmdLineSpecified && key.IsGreater(&endKeyFromCmdLine)) {
      endOfSearch = true;
      break;
    }
...
void VanitySearch::getGPUStartingKeys(int thId, int groupSize, int nbThread, Int *keys, Point *p) {
  ...
  // If a range is specified, set up threads for contiguous search
  if (startKeyFromCmdLineSpecified && endKeyFromCmdLineSpecified) {
    keys[0].Set(&startKeyFromCmdLine);
    Int step((uint64_t)STEP_SIZE);
    for (int i = 1; i < nbThread; i++) {
      keys[i].Set(&keys[i-1]);
      keys[i].Add(&step);
    }
  } else { // Original logic for random or non-range search
    for (int i = 0; i < nbThread; i++) {
      if (rekey > 0) {
        keys[i].Rand(256);
      } else {
        if (startKeyFromCmdLineSpecified) {
          keys[i].Set(&startKeyFromCmdLine);
        } else {
          keys[i].Set(&startKey);
        }
        // Only apply thread offsets if we are not doing a range search
        if (!endKeyFromCmdLineSpecified) {
          Int offT((uint64_t)i);
          offT.ShiftL(80);
  ...
  // Compute public keys for all threads
  for (int i = 0; i < nbThread; i++) {
    Int k(keys + i);
...
void VanitySearch::FindKeyGPU(TH_PARAM *ph) {
  ...
  while (ok && !endOfSearch) {
    if (endKeyFromCmdLineSpecified && keys[0].IsGreater(&endKeyFromCmdLine)) {
      endOfSearch = true;
      break;
    }
    ...
    for (int i = 0; i < (int)found.size() && !endOfSearch; i++) {
      ITEM it = found[i];
      Int keyForCheck(keys[it.thId]);
      keyForCheck.Add((uint64_t)(g.GetGroupSize() / 2));
      checkAddr(*(prefix_t*)it.hash, it.hash, keyForCheck, it.incr, it.endo, it.mode);
    }

    if (ok) {
      // In a range search, all threads jump by the total step size
      if (startKeyFromCmdLineSpecified && endKeyFromCmdLineSpecified) {
        Int totalStep((uint64_t)nbThread * STEP_SIZE);
        for (int i = 0; i < nbThread; i++) {
          keys[i].Add(&totalStep);
        }
      } else { // Original increment logic
        for (int i = 0; i < nbThread; i++) {
