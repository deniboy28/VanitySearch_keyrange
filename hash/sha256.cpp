...
#define READBE32(ptr) *(uint32_t *)(ptr)
  ...
  void Transform(uint32_t* s, const unsigned char* chunk)
  {
    ...
    Round(c, d, e, f, g, h, a, b, 0xbef9a3f7, w14 = w14 + s1(w12) + w7 + s0(w15));
    Round(b, c, d, e, f, g, h, a, 0xc67178f2, w15 = w15 + s1(w13) + w8 + s0(w0));
...
void sha256_checksum(uint8_t *input, int length, uint8_t *checksum) {
  ...
  unsigned char hash1[32]; // To store the first SHA256 hash
  unsigned char hash2[32]; // To store the second SHA256 hash
  ...
  // Second SHA256
  CSHA256 sha2;
  sha2.Write(hash1, 32); // Hash the first SHA256 output
  sha2.Finalize(hash2);
