...
void Secp256K1::GetHash160(int type,bool compressed,
  ...
  unsigned char publicKeyBytes[65]; // Max size for uncompressed public key (1 byte prefix + 32 bytes X + 32 bytes Y)
  unsigned char temp_sh[32]; // Temporary buffer for SHA256 output

  // Process k0
  if (type == P2PKH || type == BECH32) {
    if (!compressed) {
      publicKeyBytes[0] = 0x04;
      k0.x.Get32Bytes(publicKeyBytes + 1);
      k0.y.Get32Bytes(publicKeyBytes + 33);
      sha256(publicKeyBytes, 65, temp_sh);
      ripemd160_32(temp_sh, h0);
    } else {
      publicKeyBytes[0] = k0.y.IsEven() ? 0x02 : 0x03;
      k0.x.Get32Bytes(publicKeyBytes + 1);
      sha256(publicKeyBytes, 33, temp_sh);
      ripemd160_32(temp_sh, h0);
    }
  } else if (type == P2SH) {
    unsigned char kh0[20];
    GetHash160(P2PKH, compressed, k0, kh0); // Get P2PKH hash for P2SH script
    unsigned char script[22];
    script[0] = 0x00;
    script[1] = 0x14;
    memcpy(script + 2, kh0, 20);
    sha256(script, 22, temp_sh);
    ripemd160_32(temp_sh, h0);
  }
  ...
  if (type == P2PKH || type == BECH32) {
    if (!compressed) {
      publicKeyBytes[0] = 0x04;
      k1.x.Get32Bytes(publicKeyBytes + 1);
      k1.y.Get32Bytes(publicKeyBytes + 33);
      sha256(publicKeyBytes, 65, temp_sh);
      ripemd160_32(temp_sh, h1);
    } else {
      publicKeyBytes[0] = k1.y.IsEven() ? 0x02 : 0x03;
      k1.x.Get32Bytes(publicKeyBytes + 1);
      sha256(publicKeyBytes, 33, temp_sh);
      ripemd160_32(temp_sh, h1);
    }
    ...
    GetHash160(P2PKH, compressed, k1, kh1);
    unsigned char script[22];
    script[0] = 0x00;
    script[1] = 0x14;
    memcpy(script + 2, kh1, 20);
    sha256(script, 22, temp_sh);
    ripemd160_32(temp_sh, h1);
  }
  ...
  if (type == P2PKH || type == BECH32) {
    if (!compressed) {
      publicKeyBytes[0] = 0x04;
      k2.x.Get32Bytes(publicKeyBytes + 1);
      k2.y.Get32Bytes(publicKeyBytes + 33);
      sha256(publicKeyBytes, 65, temp_sh);
      ripemd160_32(temp_sh, h2);
    } else {
      publicKeyBytes[0] = k2.y.IsEven() ? 0x02 : 0x03;
      k2.x.Get32Bytes(publicKeyBytes + 1);
      sha256(publicKeyBytes, 33, temp_sh);
      ripemd160_32(temp_sh, h2);
    }
    ...
    GetHash160(P2PKH, compressed, k2, kh2);
    unsigned char script[22];
    script[0] = 0x00;
    script[1] = 0x14;
    memcpy(script + 2, kh2, 20);
    sha256(script, 22, temp_sh);
    ripemd160_32(temp_sh, h2);
  }
  ...
  if (type == P2PKH || type == BECH32) {
    if (!compressed) {
      publicKeyBytes[0] = 0x04;
      k3.x.Get32Bytes(publicKeyBytes + 1);
      k3.y.Get32Bytes(publicKeyBytes + 33);
      sha256(publicKeyBytes, 65, temp_sh);
      ripemd160_32(temp_sh, h3);
    } else {
      publicKeyBytes[0] = k3.y.IsEven() ? 0x02 : 0x03;
      k3.x.Get32Bytes(publicKeyBytes + 1);
      sha256(publicKeyBytes, 33, temp_sh);
      ripemd160_32(temp_sh, h3);
    }
    ...
    GetHash160(P2PKH, compressed, k3, kh3);
    unsigned char script[22];
    script[0] = 0x00;
    script[1] = 0x14;
    memcpy(script + 2, kh3, 20);
    sha256(script, 22, temp_sh);
    ripemd160_32(temp_sh, h3);
  }
}
